...
namespace TC_Insitu_Monitor.BLL
{
    public class Counter
    {
        ...
        private CommendStruct commendTempBefore = new CommendStruct();
        ...
        private void SerialPortDataReceivedProcess(object sender, byte[] arrDataReceived)
        {
            ...
            {                
                //Console.WriteLine(Thread.CurrentThread.ManagedThreadId.ToString() + isFirstStart + "Receive  " + DateTime.Now.Second + "  " + DateTime.Now.Ticks);
                    ...
                    foreach (var configs in configsDictionary)
                    {
                        ...
                        if (configs.Key == uSB_Interface.Comport)
                        {
                            ...
                            if(_conterState == ConterEnum.Formal)
                                tempImpParsing.Parsing(configs.Value, commendOut, commendTempBefore);
                            else
                                tempImpParsing.Parsing(configs.Value, commendOut, commendOut);

                            USBStruct newImpStruct = tempImpParsing.ImpOutput;
                            USBStruct newTempStruct = tempImpParsing.TempOutput;
                            commendTempBefore = tempImpParsing.CommendTempBefore;
                            ...
                            if (tempImpParsing.Type.Contains("Temperature"))
                            {
                                ...
                                //回傳狀態(statuses)
                                DataTempParsing datatemp = new DataTempParsing(configs.Value, newTempStruct, statuses, commendOut);
                            ...
                            else if (tempImpParsing.Type.Contains("Impedance"))
                            {
                                //傳入資料設定黨(configs.Value)/解析後數值(newTempStruct)/狀態(statuses)/電壓(commendOut)
                                //回傳狀態(statuses)
                                DataImpParsing dataimp = new DataImpParsing(configs.Value, newImpStruct, statuses, commendOut);
        ...
        {                   
            Task.Factory.StartNew(() => {
                ...
                {
                    DataConfigsStatus[] totalFormatStruct = totalStruct.ToArray();
